...
import { Play, MessageSquare, BarChart3, Users, Zap, Layout, Sparkles, Mail, Smartphone } from "lucide-react";
  ...
  return (
    <section className="relative pt-24 pb-20 overflow-hidden bg-white">
      ...
      <div className="container relative z-10 px-6 md:px-10">
        ...
        {/* Floating Contact Hub - Added for User Request */}
        <motion.div 
          initial={{ opacity: 0, y: 40 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 1, duration: 0.8 }}
          className="absolute -bottom-6 right-6 md:right-10 hidden md:flex flex-col md:flex-row items-center gap-3 z-40"
        >
          <div className="flex items-center gap-3 p-2 bg-white/50 backdrop-blur-md border border-white/40 rounded-3xl shadow-2xl">
            <a 
              href="https://wa.me/918788244416" 
              target="_blank" 
              rel="noopener noreferrer"
              className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-[#25D366] hover:bg-[#25D366] hover:text-white transition-all shadow-sm hover:shadow-xl hover:-translate-y-1 active:scale-95 group"
            >
              <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.414 0 .004 5.411.001 12.049a11.81 11.81 0 001.592 5.92L0 24l6.117-1.605a11.772 11.772 0 005.925 1.598h.005c6.635 0 12.046-5.411 12.049-12.05.003-3.216-1.252-6.241-3.535-8.524"/>
              </svg>
            </a>
            <a 
              href="mailto:rohitp2001k@gmail.com"
              className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-slate-500 hover:bg-slate-900 hover:text-[#9effa9] transition-all shadow-sm hover:shadow-xl hover:-translate-y-1 active:scale-95"
            >
              <Mail className="w-5 h-5" />
            </a>
            <a 
              href="tel:+918788244416"
              className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-slate-500 hover:bg-[#9effa9] hover:text-slate-950 transition-all shadow-sm hover:shadow-xl hover:-translate-y-1 active:scale-95"
            >
              <Smartphone className="w-5 h-5" />
            </a>
          </div>
          <div className="bg-slate-950 px-5 py-3 rounded-2xl shadow-xl hidden md:block">
            <span className="text-[10px] font-black text-[#9effa9] uppercase tracking-[0.2em] whitespace-nowrap">Connect Direct</span>
          </div>
        </motion.div>
      </div>
    </section>
